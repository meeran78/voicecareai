...
type props = {
    doctorAgent : doctorAgent,setSelectedDrs:any,selectedDr:any
}
...
function SuggestedDrsCard({doctorAgent, setSelectedDrs, selectedDr} : props) {
  ...
  return (
    <div className={`flex flex-col items-center  border rounded-2xl p-3 
        shadow-2xl hover:border-blue-600}   ${selectedDr?.id == doctorAgent.id && 'border-blue-600'}`}
    onClick={() => setSelectedDrs(doctorAgent)}  
    >
        <Image src={doctorAgent.image} alt={doctorAgent.specialist} width={80} height={80} 
